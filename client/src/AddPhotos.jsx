...
function AddPhotos() {
  ...
  const [allOriginalPhotos, setAllOriginalPhotos] = useState([]);
  ...
  const loadAvailablePhotos = async () => {
    ...
    try {
      ...
      setAllOriginalPhotos(availablePhotos);
      setDisplayedPhotos(availablePhotos.slice(0, PHOTOS_PER_PAGE));
      setCurrentPage(1);
  ...
  const loadMorePhotos = () => {
    ...
    const nextPhotos = allOriginalPhotos.slice(0, nextPage * PHOTOS_PER_PAGE);
  ...
  const filteredPhotos = displayedPhotos.filter(photo =>
    photo.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    photo.id.toString().includes(searchTerm)
  );
        ...
        <div className="posts-list" style={{ maxWidth: 'none', margin: '0' }}>
          ...
          {filteredPhotos.length === 0 ? (
            ...
            <>
              ...
              {displayedPhotos.length < allOriginalPhotos.length && (
                <div style={{ textAlign: 'center', marginTop: '20px' }}>
                  <button onClick={loadMorePhotos} className="action-btn load-more-btn">
                    טען עוד תמונות
                  </button>
                </div>
              )}
